/* 주제: 리플랙션 API 사용법 - 클래스의 메서드 정보 추출하기III
...
//    m = clazz.getMethod("m1", Class.forName("java.lang.Thread"));  파라미터로 Thread를 받는 m1메서드가 없다.(cp오류)
    System.out.println(m);
    
    m = clazz.getMethod("m1", String.class);
    System.out.println(m);
    
//    자바 원시타입(primitive)도 "class" 라는 내장 스태틱 변수가 있다.
    m = clazz.getMethod("m1", int.class);
    System.out.println(m);
    //primitive타입은 getMethod 못쓴다.
