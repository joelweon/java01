...
/* 예외 처리 - 사용자 정의 예외 클래스 만들기
=> 기존의 예외 클래스를 사용할 수도 있다.
   그러나 애플리케이션에서 발생되는 예외에 딱 맞는 클래스가 없다면,
   사용자 정의 클ㄹ래스를 만들어 사용할 수 있다.
   그냥 기존의 클래스를 사용하지, 왜 새 클래스를 만드는가?
   이점: 클래스 이름만으로 어떤 예외인지 바로 짐작할 수 있다.(가장 큰 이점)
   실행 중에 예외가 발생했을 때 개발자는 어떤 상황에서 예외가 발햇했는지
   바로 짐작가능.
